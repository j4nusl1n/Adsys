...
<?php session_start(); ?>
<html>
<head><title>Beacon Management</title>
<link rel="stylesheet" type="text/css" href="css/style.css">
<script>
	function askChange(bid){
		var new_group=document.getElementById("new_group").value;
		if(confirm("Change to Group "+new_group+" ?")==true){
			window.open("b_manage_fin.php?bid="+bid+"&new_group="+new_group, "", "height=300, width=300, menubar=no");
...
<?php
	...
	if(isset($_SESSION['current_uid'])){
		...
		echo "<h2>Beacon List</h2>";
		echo "<p><table frame='border' rules='all'><tbody>";
		echo "<tr><td>Beacon ID</td><td>Group</td><td>Change Group (0 for no group)</td></tr>";
		while($row=mysqli_fetch_row($result)){
			echo "<tr><td>Beacon $row[0]</td><td>".($row[1]==0?'No Group':"Group $row[1]")."</td><td><input type='text' name='new_group' id='new_group'><button onclick='askChange(".$row[0].")'>Change!</button></td></tr>";
		}
		echo "</tbody></table></p>";
	}
	else{
